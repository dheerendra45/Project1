      ...
      >
        <div className="flex items-center gap-4 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <h4 className="text-orange-500 text-xs md:text-sm font-medium leading-tight">
              {person.role}
            </h4>
          </div>
        </div>
        ...
        <button
          ...
          className="self-start text-sm text-orange-500 hover:text-orange-500 font-medium hover:underline transition-colors"
        >
          Read More →
        </button>
      </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {committee.members.map((member, idx) => {
              ...
              const director = directors.find(d => 
                memberName.includes(d.name.split(" ")[0]) && 
                memberName.includes(d.name.split(" ").slice(-1)[0])
              ) || {
                name: memberName,
                role: member.split(",")[1]?.trim() || "Member",
                description: `${memberName} is a member of the ${committee.name}.`,
                img: `https://via.placeholder.com/150x150/4F46E5/FFFFFF?text=${memberName.charAt(0)}`
              };
              
              return renderCard(director, idx);
            })}
          </div>
        </motion.div>
      ))}
